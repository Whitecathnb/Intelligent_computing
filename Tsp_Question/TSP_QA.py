...
def Fitness_value(old_answer,city):
    ...
    for i in range(len(old_answer)):
        ...
        # 回到初始点
        tmp_distance+=np.linalg.norm(city2-np.array(dict_city[old_answer[i][0]]))
...
def visualization(Best,city):
    ...
    # 注意加上回到初始点
    print("Walking(travel) Path:{}->{}".format(Walking_path,Best[0][0]))
    ...
    for i in range(len(Best[0])-1):
        ...
        dy_all.append(dy)
    # 注意回到初始点
    x0 = dict_city[Best[0][0]][0]
    y0 = dict_city[Best[0][0]][1]
    x.append(x0)
    y.append(y0)
    dx_all.append(x0-x2)
    dy_all.append(y0-y2)
